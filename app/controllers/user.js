...
router.post('/', function(req, res) {
    ...
    if (!checkLength(account, 5, 15) || !checkLength(password, 5, 15) || !/[0,1]/g.test(sex)) {
        res.json({
            statusCode: 1,
            message: '参数不符合要求'
        });
...
router.post('/modify', function(req, res) {
    ...
    if (!checkLength(account, 5, 15) || !/[0,1]/g.test(sex)) {
        res.json({
            statusCode: 1,
            message: '参数不符合要求'
        });
...
router.post('/password/modify', function(req, res) {
    ...
    if (utils.hashPassword(originPassword) !== req.session.user.password) {
        res.json({
            statusCode: 0,
            resultCode: -1,
            message: '密码错误'
